//====================================================================
// TrioBEASTIE_folded_trihet_alt.stan
...
real computeElem(int[,,] count,int[,] het,int site,
   int MI,int FI,int CI,real MP,real FP,real CP, int[] isPhased) 
{
   ...
   if(isPhased[site])
      return binom_lpmf(MC | MN,het[site,1],MP) // Mother
...
real modeLik(int site,int[,] mode,int[,,] count,int[,] het, real p, int[] isPhased) 
{
   ...
   real s = computeElem(count,het,site,mCopy,fCopy,cCopy,mP,fP,cP,isPhased);
   return s;
}
   ...
   real logRecomb,real logNoRecomb,real p,int N_SITES, int[] isPhased) 
{
   ...
   priors+=recomb;
   priors+=numDenovos*logDenovo + (2-numDenovos)*logNoDenovo;
   ...
   priors+=numParentAffected*logAffected+numParentUnaffected*logUnaffected;

   real delta_pos_lik = 0.5;
   real delta_neg_lik = 0.5;

   for(site in 1:N_SITES) {
      delta_pos_lik += modeLik(site,mode,count,het,p,isPhased);
      delta_neg_lik += modeLik(site,mode,count,het,1-p,isPhased);
   }
   
   return priors + log_sum_exp(delta_pos_lik, delta_neg_lik);
}

real recover_delta(int[,] mode, int[,,] count, int[,] het, 
               real p, int N_SITES, int[] isPhased){
   ...
   real delta_pos_lik_exp = exp2(delta_pos_lik);
   real delta_neg_lik_exp = exp2(delta_neg_lik);
   real denom = delta_pos_lik_exp + delta_neg_lik_exp;
   return delta_pos_lik_exp/denom;
}
}
...
parameters 
{
   real<lower=1.001,upper=1000> theta; // amount of ASE, one directional, in non-log space (range in log space is 0-10)
}

transformed parameters 
{
   real p = theta / (1 + theta); //calculate p based on theta
...
{
   // Priors:
   // how to specify half normal according to stan forum:
   // https://discourse.mc-stan.org/t/half-normal-half-cauchy-and-half-t/17314/8
   log2(theta) ~ normal(0, 1); // theta
...
generated quantities 
{
   real numerator = likelihoods(mode,count,het,logAffected,logUnaffected,
      logDenovo,logNoDenovo,logRecomb,logNoRecomb,p,N_SITES,isPhased);
   real delta_pos = recover_delta(mode,count,het,p,N_SITES,isPhased);

   real delta_neg = 1-delta_pos;
}
